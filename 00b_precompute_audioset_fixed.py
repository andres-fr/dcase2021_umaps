...
"""
Precompute AudioSet fixed representations (logSTFT, logMel)
"""
...
STFT_OUTPATH = os.path.join(
    CONF.OUT_DIR,
    f"audioset_wavnorm={CONF.WAV_NORM}_stft_win{CONF.STFT_WINSIZE}_" +
    f"hop{CONF.STFT_HOPSIZE}.h5")
MEL_OUTPATH = os.path.join(
    CONF.OUT_DIR,
    f"audioset_wavnorm={CONF.WAV_NORM}_mel_win{CONF.STFT_WINSIZE}_" +
...
paths = list(Path(CONF.ROOT_PATH).glob("*.wav"))
