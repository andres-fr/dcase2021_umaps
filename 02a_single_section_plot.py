...
#
CONF.LEGEND_SHADOW_SIZE = 250
CONF.LEGEND_DOT_RATIO = 0.4
...
source_dot_marker = plt.Line2D(
    ...
    color="black", markerfacecolor="white", linestyle="none",
...
# get handles and add 2 extra
ax_l_handles, ax_l_labels = ax_l.get_legend_handles_labels()
ax_l_handles.extend([source_dot_marker, target_dot_marker])
ax_l_labels.extend([CONF.TEST_SOURCE_NORMAL_LABEL,
                    CONF.TEST_TARGET_NORMAL_LABEL])
# create legend and make them opaqu
leg = ax_l.legend(ax_l_handles, ax_l_labels, borderpad=0.4, labelspacing=0.5,
...
leg = ax_r.legend(ax_r_handles, ax_r_labels, borderpad=0.4, labelspacing=0.5,
                  prop={'size': CONF.LEGEND_FONT_SIZE})
for lh in leg.legendHandles:
    if isinstance(lh, PathCollection):
        fc_arr = lh.get_fc().copy()
